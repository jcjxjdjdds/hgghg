   ...
   try:
    ...
    if message.new_chat_members[0].username == "ALH_KAR":
      try:
         ...
         await client.set_administrator_title(chat_id, user_id, "مرعب")
      except:
        pass
      return await message.reply_text(f"**♪ انضم المطو مرعب للشات  🚦 .\n♪ مرحبا بك : @ALH_KAR  🚦 .**")
    dev = await get_dev(bot_username)
...
async def ALH_KAR(client, message):
  bot_username = client.me.username
  dev = await get_dev(bot_username)
...
"مره واحد اسمو مرعب  كان بيرجم ابليس ف الحج قالولو ليه؟ قال عشان يمكن احتاجو 👻😹",
...
async def alive(client: Client, message):
    ...
    alive = f"""╭──── • ◈ • ────╮
么 [𝗦𝗼𝘂𝗿𝗰𝗲 𝗔𝗹𝗠𝗮𝗿𝗶𝗯](t.me/HLV_M) 🚦 .
么  [𝒅𝒆𝒗 𝙼𝙾𝚁𝙷𝙴𝙱 ](t.me/ALH_KAR) 🚦 .
╰──── • ◈ • ────╯
    ...
    await message.reply_photo(
        ...
        reply_markup=InlineKeyboardMarkup(
                    [
                        ...
                        [
                            InlineKeyboardButton("SeMo ElKbEr 🥷", user_id=f"ALH_KAR")
                        ],
                        [
...
@Client.on_message(filters.command(["جوز كيان مرعب","محمود","المبرمج"], ""))
...
async def tiillli(client, message):
  if message.from_user.username in ["ALH_KAR"]:
   ...
   if message.from_user.username == "ALH_KAR":
     await message.reply_text(f"**♪ تم تنفيذ الامر بواسطة المطور مرعب  🚦 .**")
   else:
...
async def bt(client: Client, message: Message):
  try:
     ...
     if userr.username in ["ALH_KAR"]:
         await message.reply_text("**♪ رتبتك هي : المطور مرعب+  🚦 .**")
         return
     if userr.username in ["ALH_KAR"]:
